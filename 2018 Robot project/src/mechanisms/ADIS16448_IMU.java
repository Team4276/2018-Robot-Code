...
public class ADIS16448_IMU extends GyroBase implements Gyro, PIDSource {
	...
	/*
	 * private void calculate_heading() { // For this game Z can be ignored, so
	 * the heading vector is just magnitude and yaw angle. // The current
	 * velocity estimates are then used to produce a robot-frame heading vector.
	 * 
	 * double xVelocity = getVelocityX(); double yVelocity = getVelocityY();
	 * 
	 * double xyRobotFrameSpeed = Math.sqrt(Math.pow(xVelocity, 2) +
	 * Math.pow(yVelocity, 2)); double xyRobotFrameHeading =
	 * Math.toDegrees(Math.atan2(yVelocity, xVelocity));
	 * 
	 ...
	 * 
	 * xyRobotFrameHeading += getYaw(); xyRobotFrameHeading +=
	 * Robot.yawOffsetToFieldFrame; xyRobotFrameHeading %= 360.0;
	 * if(xyRobotFrameHeading > 180.0) { xyRobotFrameHeading -= 180.0; }
	 * Robot.xyFieldFrameHeading = xyRobotFrameHeading;
	 * SmartDashboard.putNumber("Field Frame Heading",
	 * Robot.xyFieldFrameHeading); }
	 */
	 ...
	 * @Override
	 * 
	 * public void updateTable() { ITable table = getTable(); if (table != null)
	 * { table.putNumber("Value", getAngle()); table.putNumber("Pitch",
	 * getPitch()); table.putNumber("Roll", getRoll()); table.putNumber("Yaw",
	 * getYaw()); table.putNumber("AccelX", getAccelX());
	 * table.putNumber("AccelY", getAccelY()); table.putNumber("AccelZ",
	 * getAccelZ()); table.putNumber("AngleX", getAngleX());
	 * table.putNumber("AngleY", getAngleY()); table.putNumber("AngleZ",
	 * getAngleZ()); } }
	 */
}
